...
func resourceAciBridgeDomainCreate(d *schema.ResourceData, m interface{}) error {
	...
	if relationTofvRsBDToNetflowMonitorPol, ok := d.GetOk("relation_fv_rs_bd_to_netflow_monitor_pol"); ok {
		...
		for _, relationParam := range relationParamList {
			...
			err = aciClient.CreateRelationfvRsBDToNetflowMonitorPol(fvBD.DistinguishedName,paramMap["tnNetflowMonitorPolName"].(string),paramMap["fltType"].(string))
			if err != nil {
				return err
			}
		}
...
func resourceAciBridgeDomainUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if d.HasChange("relation_fv_rs_bd_to_profile") {
		oldTdn, newTdn := d.GetChange("relation_fv_rs_bd_to_profile")
		err = aciClient.DeleteRelationfvRsBDToProfile(fvBD.DistinguishedName, oldTdn.(string))
		if err != nil {
			return err
		}
		err = aciClient.CreateRelationfvRsBDToProfile(fvBD.DistinguishedName, newTdn.(string))
		if err != nil {
			return err
		}
	...
	if d.HasChange("relation_fv_rs_bd_to_relay_p") {
		oldTdn, newTdn := d.GetChange("relation_fv_rs_bd_to_relay_p")
		err = aciClient.DeleteRelationfvRsBDToRelayP(fvBD.DistinguishedName, oldTdn.(string))
		if err != nil {
			return err
		}
		err = aciClient.CreateRelationfvRsBDToRelayP(fvBD.DistinguishedName, newTdn.(string))
		if err != nil {
			return err
		}
	...
	if d.HasChange("relation_fv_rs_abd_pol_mon_pol") {
		oldTdn, newTdn := d.GetChange("relation_fv_rs_abd_pol_mon_pol")
		err = aciClient.DeleteRelationfvRsABDPolMonPol(fvBD.DistinguishedName, oldTdn.(string))
		if err != nil {
			return err
		}
		err = aciClient.CreateRelationfvRsABDPolMonPol(fvBD.DistinguishedName, newTdn.(string))
		if err != nil {
			return err
		}
	...
	if d.HasChange("relation_fv_rs_bd_flood_to") {
		oldc, newc := d.GetChange("relation_fv_rs_bd_flood_to")
	...
	if d.HasChange("relation_fv_rs_bd_to_fhs") {
		oldTdn, newTdn := d.GetChange("relation_fv_rs_bd_to_fhs")
		err = aciClient.DeleteRelationfvRsBDToFhs(fvBD.DistinguishedName, oldTdn.(string))
		if err != nil {
			return err
		}
		err = aciClient.CreateRelationfvRsBDToFhs(fvBD.DistinguishedName, newTdn.(string))
		if err != nil {
			return err
		}
	...
	if d.HasChange("relation_fv_rs_bd_to_netflow_monitor_pol") {
		oldc, newc := d.GetChange("relation_fv_rs_bd_to_netflow_monitor_pol")
		oldRelSet := oldc.(*schema.Set)
		newRelSet := newc.(*schema.Set)
		...
		for _, relDn := range relToDelete {
			err = aciClient.DeleteRelationfvRsBDToNetflowMonitorPol(fvBD.DistinguishedName, relDn)
			if err != nil {
				return err
			}
		...
		for _, relDn := range relToCreate {
			err = aciClient.CreateRelationfvRsBDToNetflowMonitorPol(fvBD.DistinguishedName, relDn)
			if err != nil {
				return err
			}
	...
	if d.HasChange("relation_fv_rs_bd_to_out") {
		oldc, newc := d.GetChange("relation_fv_rs_bd_to_out")
		oldRelSet := oldc.(*schema.Set)
		newRelSet := newc.(*schema.Set)
