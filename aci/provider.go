...
func Provider() terraform.ResourceProvider {
	return &schema.Provider{
		...
		ResourcesMap: map[string]*schema.Resource{
			"aci_fv_tenant": resourceAciTenant(),
			"aci_fv_ap":     resourceAciApplicationProfile(),
			"aci_fv_bd":     resourceAciBridgeDomain(),
			"aci_vz_br_cp":  resourceAciContract(),
			"aci_fv_ae_pg":  resourceAciApplicationEPG(),
			"aci_vz_subj":   resourceAciContractSubject(),
			"aci_fv_subnet": resourceAciSubnet(),
			"aci_vz_filter": resourceAciFilter(),
			"aci_vz_entry":  resourceAciFilterEntry(),
