...
func TestAccAciBridgeDomain_Basic(t *testing.T) {
	...
	description := "fv_bd created while acceptance testing"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAciBridgeDomainExists("aci_fv_bd.foofv_bd", &bridge_domain),
...
func testAccCheckAciBridgeDomainAttributes(fv_tenant_name, fv_bd_name, description string, bridge_domain *models.BridgeDomain) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if fv_tenant_name != GetMOName(bridge_domain.DistinguishedName) {
			return fmt.Errorf("Bad fv_tenant %s", GetMOName(bridge_domain.DistinguishedName))
		}
